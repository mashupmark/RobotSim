...
//        System.out.println("method not accessible");
         ...
         * This is the only part in RobotSim where the motors can be parallelized.
         *
        ...
        public void act() {
            ...
            parts.stream().filter(part -> part instanceof LightSensor).forEach(part -> ((LightSensor) part).notifyEvent());

            // ------------------ We notify ultrasonic listeners -------------
            parts.stream().filter(part -> part instanceof UltrasonicSensor).forEach(part -> ((UltrasonicSensor) part).notifyEvent());
            ...
            // ------------------ We have a gear --------------------
            Thread gearThread = new Thread(() -> updateGear(gear));
            gearThread.start();

            // ------------------ We have two motors --------------
            Thread motorThread = null;
            if (motors.size() == 2) {
                motorThread = new Thread(() -> updateMotors(motors));
                motorThread.start();
            }

            if (motors.size() == 1)  // we have 1 motor
            {
                motorThread = new Thread(() -> updateMotor(motors));
                motorThread.start();
            }
